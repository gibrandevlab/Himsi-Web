...
class AbsenceService extends Service
{
    ...
    {
        return $this->meetAbsenceRepository->joinMeet()->first(array("meet.meetName", "meet_absence.created_at", "meet.id"));
    }
    ...
    {
        return [
            "meet" => $this->meetAbsenceRepository->getMeetAbsenceByID($id)->join("member", "member.id", "=", "meet_absence.member_id")->join("meet", "meet.id", "=", "meet_absence.meet_id")->get(array("meet_absence.id", "meet.meetName", "meet.created_at", "name", "nim"))->first(),
            "reasons" => array("sakit", "ijin", "absen", "hadir")
        ];
    }
    public function detailMeetAbsence(string $id)
    {
        return $this->meetAbsenceRepository->joinMemberMeet()->where("meet_absence.id", "=", $id)->select("meet.meetName", "meet.id", "meet.material", "meet.location", "meet_absence.status",  "meet_absence.created_at", "member.name", "member.nim")->first();
    }
    ...
    {
        return array(
            "events" => $this->eventRepository->query()->where("status", "=", false)->get(array("id", "eventName", "created_at")),
    ...
    {
        return $this->eventAbsenceRepository->query()->where("event_absence.event_id", "=", $id)->join("event", "event.id", "=", "event_absence.event_id")->get(array("eventName", "event.startAt AS created_at", "event.id"))->first();
    }
